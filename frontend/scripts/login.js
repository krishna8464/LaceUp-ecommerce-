...
try {
  let res = await fetch("http://localhost:6060/user/admin/allusers")
...
function loginreq(){
    ...
    <div id="alert"></div>
    `
    if(document.getElementById('mail_inbox').value !== ""){
    cont.innerHTML=data;
    document.getElementById("mail_inbox").readOnly=true;
    }
}
...
async function loginbtn(){
    ...
    let str = emailbox.value.toLowerCase();
    let status3=str.includes("@gmail.com")
    if(status3 === false){
        let data=`
        <br>
    ...
    try {
        let res = await await fetch("http://localhost:6060/user/login",{
            method:"POST",
            headers:{
      ...
      sessionStorage.setItem("userdetials",JSON.stringify(user_det))
        sessionStorage.setItem("Access_tocken",out.Access_Token)
